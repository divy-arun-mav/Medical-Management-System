...
export const Web3Provider = ({ children }) => {
    ...
    const setDoctor = async (_ic, _name, _phone, _gender, _dob, _qualification, _major) => {
        try {
            ...
            const tx = await contract.connect(account).setDoctor(_ic, _name, _phone, _gender, _dob, _qualification, _major);
            await tx.wait();
        } catch (error) {
    ...
    const setDetails = async (_ic, _name, _phone, _gender, _dob, _height, _weight, _houseaddr, _bloodgroup, _allergies, _medication) => {
        try {
            const { contract, account } = state;
            const tx = await contract.connect(account).setDetails(_ic, _name, _phone, _gender, _dob, _height, _weight, _houseaddr, _bloodgroup, _allergies, _medication);
            await tx.wait();
        } catch (error) {
    ...
    const setAppointment = async (patientAddress, date, time, diagnosis, prescription, description, status) => {
        try {
            const { contract, account } = state;
            const tx = await contract.connect(account).setAppointment(patientAddress, date, time, diagnosis, prescription, description, status);
            await tx.wait();
        } catch (error) {
